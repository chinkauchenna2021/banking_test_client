...
export default function ManualDepositsPage() {
  ...
  const handleUploadProof = async () => {
    ...
    try {
      ...
      toast.success('Proof uploaded successfully. Your deposit is now awaiting confirmation.');
  ...
  const handleCreateDeposit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedMethod === 'bank_transfer') {
        deposit = await quickBankTransfer(
          amount,
        ...
        deposit = await quickCryptoDeposit(
          amount,
          depositData.account_id,
          {
        ...
        toast.error('Selected method not implemented');
        return;
      }
  ...
  const getDepositStats = () => {
    return {
      ...
      awaiting: deposits.filter(d => d.status === 'awaiting_confirmation' || d.status === 'awaiting_proof').length,
      completed: deposits.filter(d => d.status === 'completed' || d.status === 'confirmed').length,
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Manual Deposits</h1>
            <p className="text-muted-foreground">
              {getDepositStats().total} requests • {formatCurrency(getDepositStats().totalAmount)} total deposited
            </p>
          </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-3">
                        ...
                        {companyAccount.account_name && (
                          <div>
                            ...
                            <div className="font-medium">{companyAccount.account_name}</div>
                          </div>
                        )}
                        {companyAccount.account_number && (
                          <div>
                            <Label className="text-sm text-muted-foreground">Account Number</Label>
                            <div className="font-mono font-medium">{companyAccount.account_number}</div>
                          </div>
                        )}
                        {companyAccount.bank_name && (
                          <div>
                            <Label className="text-sm text-muted-foreground">Bank Name</Label>
                            <div className="font-medium">{companyAccount.bank_name}</div>
                          </div>
                        )}
                        {companyAccount.swift_code && (
                          <div>
                            <Label className="text-sm text-muted-foreground">SWIFT Code</Label>
                            <div className="font-mono font-medium">{companyAccount.swift_code}</div>
                          </div>
                        )}
                        {companyAccount.wallet_address && (
                          <div>
                            <Label className="text-sm text-muted-foreground">Wallet Address</Label>
                            <div className="font-mono text-sm break-all">{companyAccount.wallet_address}</div>
                          </div>
                        )}
                        {companyAccount.network && (
                          <div>
                            <Label className="text-sm text-muted-foreground">Network</Label>
                            <div className="font-medium">{companyAccount.network}</div>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        {companyAccount.instructions && companyAccount.instructions.length > 0 && (
                          <div className="space-y-2">
                            {companyAccount.instructions.map((instruction, index) => (
                        ...
                        <div className="text-sm text-muted-foreground">
                          After making the transfer, upload proof of payment using the "Upload Proof" button.
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-2xl font-bold">{getDepositStats().total}</div>
                    </div>
              ...
              <Card>
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-muted-foreground">Awaiting Proof</p>
                      <div className="text-2xl font-bold">{getDepositStats().awaiting}</div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 w-full md:w-auto">
                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full md:w-auto">
                      <TabsList>
                        ...
                        <TabsTrigger value="awaiting_confirmation">Awaiting</TabsTrigger>
                          ...
                          <TableCell className="text-right">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                {deposit.receipt_url && (
                                  <DropdownMenuItem onClick={() => downloadReceipt(deposit.id)}>
                                    <Receipt className="h-4 w-4 mr-2" />
                                    Download Receipt
                                  </DropdownMenuItem>
                                )}
                                
                                <DropdownMenuItem onClick={() => shareDeposit(deposit.id)}>
          ...
          <form onSubmit={handleCreateDeposit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setDepositData({...depositData, account_id: value})}
                required
              >
                <SelectTrigger>
              ...
              <>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setDepositData({...depositData, sender_bank: e.target.value})}
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setDepositData({...depositData, sender_account: e.target.value})}
                  />
                </div>
              </>
            )}
            ...
            {selectedMethod === 'crypto' && (
              <>
                <div className="space-y-2">
                  <Label htmlFor="crypto_address">From Crypto Address (Optional)</Label>
                  <Input
                    id="crypto_address"
                    placeholder="0x..."
                    value={depositData.crypto_address}
                    onChange={(e) => setDepositData({...depositData, crypto_address: e.target.value})}
                  />
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="crypto_transaction_hash">Transaction Hash (Optional)</Label>
                  <Input
                    id="crypto_transaction_hash"
                    placeholder="Transaction ID"
                    value={depositData.crypto_transaction_hash}
                    onChange={(e) => setDepositData({...depositData, crypto_transaction_hash: e.target.value})}
                  />
                </div>
              </>
            )}
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) => setDepositData({...depositData, additional_notes: e.target.value})}
              />
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
              ...
              <p className="text-xs text-blue-700">
                After submitting this request, transfer the funds to the company account shown above and upload proof of payment.
              </p>
            </div>
      ...
      <Dialog open={showProofDialog} onOpenChange={setShowProofDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Upload proof of payment for deposit: {selectedDeposit?.deposit_reference}
            </DialogDescription>
          </DialogHeader>
            ...
            {selectedDeposit && (
              <div className="bg-gray-50 p-4 rounded-lg space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold">{formatCurrency(selectedDeposit.amount)}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-sm">{selectedDeposit.deposit_reference}</span>
                </div>
              </div>
            )}
        ...
        <Dialog open={!!selectedDeposit} onOpenChange={() => setSelectedDeposit(null)}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Complete information for deposit {selectedDeposit.deposit_reference}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="font-mono">{selectedDeposit.deposit_reference}</div>
                </div>
                <div>
                  <Label className="text-sm text-muted-foreground">Date Created</Label>
                  <div>{format(new Date(selectedDeposit.created_at), 'PPP p')}</div>
                </div>
                <div>
              ...
              <div className="border-t pt-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="text-sm text-muted-foreground">Deposit Method</Label>
                    <div className="font-medium">{getMethodText(selectedDeposit.method)}</div>
                  </div>
                  <div>
                    <Label className="text-sm text-muted-foreground">To Account</Label>
                    <div>
                      <div className="font-medium">{selectedDeposit.account?.account_name}</div>
                      <div className="text-sm text-muted-foreground">
              ...
              <div className="border-t pt-4">
                ...
                {(selectedDeposit.sender_name || selectedDeposit.sender_bank || selectedDeposit.sender_account) && (
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {selectedDeposit.sender_bank && (
                      <div>
                        <Label className="text-sm text-muted-foreground">Sender Bank</Label>
                        <div>{selectedDeposit.sender_bank}</div>
                      </div>
                    )}
                    {selectedDeposit.sender_account && (
                      <div>
                        <Label className="text-sm text-muted-foreground">Sender Account</Label>
                ...
                {selectedDeposit.confirmed_at && (
                  <div className="mt-3">
                    ...
                    <div>{format(new Date(selectedDeposit.confirmed_at), 'PPP p')}</div>
                  </div>
                )}
              </div>
              ...
              <div className="border-t pt-4 flex justify-end gap-3">
                {selectedDeposit.receipt_url && (
                  <Button variant="outline" onClick={() => downloadReceipt(selectedDeposit.id)}>
                    <Receipt className="h-4 w-4 mr-2" />
                    Download Receipt
                  </Button>
                )}
                {(selectedDeposit.status === 'pending' || selectedDeposit.status === 'awaiting_confirmation') && (
                  <Button onClick={() => {
