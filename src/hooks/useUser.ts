...
export const useUser = () => {
  ...
  const updateUserPreference = useCallback(async (key: string, value: any) => {
  ...
  return {
    ...
    formattedBalance: user?.balance ? formatCurrency(parseFloat(user.balance)) : '$0.00',
...
export const useUserSettings = () => {
  ...
  const updateSecurityPreferences = useCallback(async (preferences: {
  ...
  const submitKYCDocuments = useCallback(async (
    ...
    formData.append('identification_image', identificationFile);
    
    if (taxId) formData.append('tax_id', taxId);
    if (monthlyIncome) formData.append('monthly_income', monthlyIncome.toString());
    if (sourceOfFunds) formData.append('source_of_funds', sourceOfFunds);
